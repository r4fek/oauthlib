...
class TestClaimsHandling(TestCase):
    ...
    def retrieve_claims_saved_with_code(
        self, client_id, code, client, request, *args, **kwargs
    ):
    ...
    def setUp(self):
        ...
        self.validator.get_default_redirect_uri.return_value = (
            TestClaimsHandling.DEFAULT_REDIRECT_URI
        )
    ...
    async def test_claims_stored_on_code_creation(self):
        ...
        claims = {
            "id_token": {"claim_1": None, "claim_2": {"essential": True}},
            "userinfo": {"claim_3": {"essential": True}, "claim_4": None},
        }
        ...
        h, b, s = await self.server.create_authorization_response(
        ...
        _, body, _ = await self.server.create_token_response(
            token_uri,
            body='client_id=me&redirect_uri=http://back.to/me&grant_type=authorization_code&code=%s'
            % code,
        )
