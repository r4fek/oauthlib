...
class TestRefreshToken(TestCase):
    ...
    def setUp(self):
        ...
        self.validator.get_original_scopes.return_value = ['test_scope']
    ...
    async def test_refresh_token_with_openid(self):
        request_body = (
            'scope=openid+test_scope&grant_type=refresh_token&refresh_token=abc'
        )
        headers, body, status = await self.server.create_token_response(
            '', body=request_body
        )
