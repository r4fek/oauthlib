...
class ResourceOwnerPasswordCredentialsGrantTest(TestCase):
    ...
    async def test_create_token_response(self):
        ...
        status_code = (await self.auth.create_token_response(self.request, bearer))[2]
    ...
    def test_custom_auth_validators_unsupported(self):
        ...
        expected = (
            'ResourceOwnerPasswordCredentialsGrant does not '
            'support authorization validators. Use token '
            'validators instead.'
        )
        with self.assertRaises(ValueError) as caught:
            ResourceOwnerPasswordCredentialsGrant(
                self.mock_validator, pre_auth=[authval1]
            )
        self.assertEqual(caught.exception.args[0], expected)
        with self.assertRaises(ValueError) as caught:
            ResourceOwnerPasswordCredentialsGrant(
                self.mock_validator, post_auth=[authval2]
            )
        self.assertEqual(caught.exception.args[0], expected)
    ...
    async def test_invalid_request_missing_params(self):
