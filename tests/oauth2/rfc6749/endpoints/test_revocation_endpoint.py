...
class RevocationEndpointTest(TestCase):
    ...
    def setUp(self):
        self.validator = AsyncMock(spec=RequestValidator())
    ...
    async def test_revoke_token(self):
        for token_type in ('access_token', 'refresh_token', 'invalid'):
            body = urlencode([('token', 'foo'), ('token_type_hint', token_type)])
            h, b, s = await self.endpoint.create_revocation_response(
    ...
    async def test_revoke_token_client_authentication_failed(self):
        self.validator.authenticate_client.return_value = False
        body = urlencode([('token', 'foo'), ('token_type_hint', 'access_token')])
        h, b, s = await self.endpoint.create_revocation_response(
            self.uri, headers=self.headers, body=body
        )
        self.assertEqual(
    ...
    async def test_revoke_token_public_client_authentication(self):
    ...
    async def test_revoke_token_public_client_authentication_failed(self):
        self.validator.client_authentication_required.return_value = False
    ...
    async def test_revoke_with_callback(self):
        ...
        for token_type in ('access_token', 'refresh_token', 'invalid'):
            ...
            h, b, s = await endpoint.create_revocation_response(
                self.uri, headers=self.headers, body=body
            )
            self.assertEqual(h, {})
    ...
    async def test_revoke_unsupported_token(self):
        endpoint = RevocationEndpoint(
            self.validator, supported_token_types=['access_token']
        )
        body = urlencode([('token', 'foo'), ('token_type_hint', 'refresh_token')])
        h, b, s = await endpoint.create_revocation_response(
            self.uri, headers=self.headers, body=body
        )
        ...
        h, b, s = await endpoint.create_revocation_response(
            self.uri, headers=self.headers, body=''
        )
        self.assertEqual(h, self.resp_h)
        self.assertEqual(loads(b)['error'], 'invalid_request')
        self.assertEqual(s, 400)

    async def test_revoke_invalid_request_method(self):
        endpoint = RevocationEndpoint(
            self.validator, supported_token_types=['access_token']
        )
        ...
        for method in test_methods:
            body = urlencode([('token', 'foo'), ('token_type_hint', 'refresh_token')])
            h, b, s = await endpoint.create_revocation_response(
                self.uri, http_method=method, headers=self.headers, body=body
            )
            self.assertEqual(h, self.resp_h)
            self.assertEqual(loads(b)['error'], 'invalid_request')
    ...
    async def test_revoke_bad_post_request(self):
        endpoint = RevocationEndpoint(
            self.validator, supported_token_types=['access_token']
        )
        for param in ['token', 'secret', 'code', 'foo']:
            ...
            body = urlencode([('token', 'foo'), ('token_type_hint', 'access_token')])
            h, b, s = await endpoint.create_revocation_response(
                uri, headers=self.headers, body=body
            )
            self.assertEqual(h, self.resp_h)
            self.assertEqual(loads(b)['error'], 'invalid_request')
            self.assertIn(
                'query parameters are not allowed', loads(b)['error_description']
            )
            self.assertEqual(s, 400)
